...
int main(){
  ...
  size_t snap_rejected=0,snap_silent=0,wal_rejected=0,wal_silent=0,trunc_safe=0,trunc_bad=0;int trials=200;
  auto snap_sz=fs::file_size(base/"snapshot.bdr3"),wal_sz=fs::file_size(base/"wal-000001.log");
  for(int t=0;t<trials;++t){fs::path d="v42s";fs::remove_all(d);fs::create_directory(d);copy_file(base/"snapshot.bdr3",d/"snapshot.bdr3");copy_file(base/"wal-000001.log",d/"wal-000001.log");size_t off=size_t(rng()%(snap_sz-4));flip_byte(d/"snapshot.bdr3",off,uint8_t(1u<<(rng()%8)));try{auto r=reopen(d);(void)r;++snap_silent;}catch(...){++snap_rejected;}}
  for(int t=0;t<trials;++t){fs::path d="v42w";fs::remove_all(d);fs::create_directory(d);copy_file(base/"snapshot.bdr3",d/"snapshot.bdr3");copy_file(base/"wal-000001.log",d/"wal-000001.log");size_t minoff=sizeof(WalHeader),off=minoff+size_t(rng()%std::max<uint64_t>(1,wal_sz-minoff));flip_byte(d/"wal-000001.log",off,uint8_t(1u<<(rng()%8)));try{auto r=reopen(d);(void)r;++wal_silent;}catch(...){++wal_rejected;}}
  for(int t=0;t<trials;++t){fs::path d="v42t";fs::remove_all(d);fs::create_directory(d);copy_file(base/"snapshot.bdr3",d/"snapshot.bdr3");copy_file(base/"wal-000001.log",d/"wal-000001.log");uint64_t cut=sizeof(WalHeader)+(rng()%std::max<uint64_t>(1,wal_sz-sizeof(WalHeader)));fs::resize_file(d/"wal-000001.log",cut);try{auto r=reopen(d);if(r.seq<=seq)++trunc_safe;else ++trunc_bad;}catch(...){++trunc_safe;}}
  bool pass=snap_rejected==size_t(trials)&&snap_silent==0&&wal_rejected==size_t(trials)&&wal_silent==0&&trunc_safe==size_t(trials)&&trunc_bad==0;
  std::cout<<"seed,trials,snapshot_rejected,snapshot_silent,wal_rejected,wal_silent,trunc_safe,trunc_bad,pass\n"<<SEED<<','<<trials<<','<<snap_rejected<<','<<snap_silent<<','<<wal_rejected<<','<<wal_silent<<','<<trunc_safe<<','<<trunc_bad<<','<<pass<<"\n";
  return pass?0:2;
}
