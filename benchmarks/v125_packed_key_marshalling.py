...
def raw_get_many_with_packed_keys(db: AtomicBDR, keys: list[str]) -> dict[str, float | int]:
    started = time.perf_counter_ns()
    raw_keys = [_to_bytes(key, field="key") for key in keys]
    if any(not raw for raw in raw_keys):
        raise ValueError("empty key")
    encoded_keys_ms = elapsed_ms(started)
    ...
    packed_pointer_table_ms = elapsed_ms(started)
    ...
    packed_key_marshalling_ms = encoded_keys_ms + packed_pointer_table_ms
    total_value_bytes = sum(len(value) for value in values if value is not None)
    return {
        "encoded_keys_ms": encoded_keys_ms,
        "packed_pointer_table_ms": packed_pointer_table_ms,
        "packed_key_marshalling_ms": packed_key_marshalling_ms,
        "native_call_ms": native_call_ms,
        "python_materialize_ms": python_materialize_ms,
        "python_free_ms": python_free_ms,
        "measured_total_ms": packed_key_marshalling_ms + native_call_ms + python_materialize_ms + python_free_ms,
        "packed_key_bytes": len(packed),
...
def run_size(count: int, library: Path, root: Path) -> dict[str, object]:
    ...
    with AtomicBDR.open(bdr_root, library_path=library) as db:
        packed = raw_get_many_with_packed_keys(db, keys)
        started = time.perf_counter_ns()
    ...
    if packed["total_value_bytes"] != expected_bytes or packed["found"] != len(keys):
        raise AssertionError(f"packed bulk read parity failed at size {count}")
    ...
    return {
        ...
        "packed": packed,
        "public_to_packed_total_ratio": public_get_many_ms / float(packed["measured_total_ms"]),
        "parity": True,
    }
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bdr-v125-") as temp_dir:
        root = Path(temp_dir)
        samples = [run_size(size, args.library, root) for size in SIZES]

    print(json.dumps({
        "benchmark": "v125_packed_key_marshalling",
        "memoria_validated_commit": MEMORIA_VALIDATED_COMMIT,
        "sizes": list(SIZES),
        "contract": {
            "measurement_only": True,
            "existing_c_abi_only": True,
