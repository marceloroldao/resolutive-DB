...
def _load_library() -> C.CDLL:
    ...
    candidates.extend([
        str(here.parent / "build" / "libbdr_c.so"),
        str(here.parent / "build" / "libbdr_c.dylib"),
    ])
    ...
    raise RuntimeError("Unable to load BDR C library. Set BDR_C_LIBRARY. Tried: " + " | ".join(errors))
...
class Database:
    ...
    def wait(self, ticket: int) -> None:
        self._require_open(); _check(_lib.bdr_wait(self._handle, ticket))

    def sync(self) -> None:
        self._require_open(); _check(_lib.bdr_sync(self._handle))

    def checkpoint(self) -> None:
        self._require_open(); _check(_lib.bdr_checkpoint(self._handle))

    @property
    def size(self) -> int:
        self._require_open(); out = C.c_size_t(); _check(_lib.bdr_size(self._handle, C.byref(out))); return int(out.value)

    @property
    def last_sequence(self) -> int:
        self._require_open(); out = C.c_uint64(); _check(_lib.bdr_last_sequence(self._handle, C.byref(out))); return int(out.value)

    @property
    def durable_sequence(self) -> int:
        self._require_open(); out = C.c_uint64(); _check(_lib.bdr_durable_sequence(self._handle, C.byref(out))); return int(out.value)
