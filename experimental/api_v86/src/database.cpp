...
static uint64_t replay_snapshot(const fs::path&p,ResolutiveIndex&index){
    ...
    // Validate the complete snapshot before mutating the live index.
    {
    ...
    // Second pass streams directly into rho -> local Robin Hood partitions.
    std::ifstream f(p,std::ios::binary);
    ...
    for(uint64_t i=0;i<n;++i){
        ...
        consumed+=8;
        const uint32_t kl=be32(lens),vl=be32(lens+4);
...
public:
    ...
    void writer_loop(){
        ...
        try{
            ...
            {std::lock_guard<std::mutex>lk(queue_mu);if(!writer_error)writer_error=std::current_exception();stop=true;}
            durable_cv.notify_all();queue_cv.notify_all();
        }
    }
    ...
    void checkpoint_impl(){
        ...
        uint64_t seq=next_seq.load();wait_impl(Ticket{seq});auto items=index.snapshot_items();auto bytes=encode_snapshot(seq,items);
        ...
        write_all(sfd,bytes.data(),bytes.size());if(::fsync(sfd)){::close(sfd);throw std::runtime_error("snapshot fsync failed");}::close(sfd);fs::rename(tmp,dst);fsync_dir(dir);
        std::lock_guard<std::mutex>wg(wal_mu);if(wal_fd>=0){if(::fsync(wal_fd))throw std::runtime_error("pre-rotation fsync failed");::close(wal_fd);wal_fd=-1;}
    ...
    void close_impl(){
        ...
        queue_cv.notify_all();durable_cv.notify_all();
        if(writer.joinable())writer.join();
        {
            std::lock_guard<std::mutex>wg(wal_mu);
            if(wal_fd>=0){if(!close_error&&::fdatasync(wal_fd))close_error=std::make_exception_ptr(std::runtime_error("BDR close fdatasync failed"));::close(wal_fd);wal_fd=-1;}
        }
