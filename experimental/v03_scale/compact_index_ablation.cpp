...
class CompactIndex {
    ...
    struct Part {
        mutable std::shared_mutex mu; std::vector<Slot> slots; std::vector<char> arena; std::size_t records=0;
        Part():slots(8){}
        ...
        bool ins(std::uint64_t fp,const std::string&k,const std::string&v){std::size_t mask=slots.size()-1,idx=start(fp,mask);Slot cur=make(fp,k,v);for(;;){auto&x=slots[idx];if(!x.used){x=cur;++records;return true;}if(x.fp==fp&&key_view(x)==k){x=cur;return false;}if(x.dist<cur.dist)std::swap(x,cur);idx=(idx+1)&mask;if(++cur.dist>=slots.size())throw std::runtime_error("overflow");}}
...
public:
    explicit CompactIndex(std::size_t n=4096){p_.reserve(n);for(std::size_t i=0;i<n;++i)p_.push_back(std::make_unique<Part>());}    
