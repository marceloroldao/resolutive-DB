...
namespace bdr {
namespace compact_candidate_detail {
...
class CompactIndex {
    ...
    struct Partition {
        ...
        static std::size_t start(std::uint64_t fp,std::size_t mask)noexcept{return std::size_t(compact_candidate_detail::mix64(fp))&mask;}
        ...
        void rehash(std::size_t cap){auto old=std::move(slots);slots.assign(compact_candidate_detail::next_pow2(cap),Slot{});records=0;for(auto s:old)if(s.used){const auto mask=slots.size()-1;auto idx=start(s.fingerprint,mask);s.dist=0;for(;;){auto&slot=slots[idx];if(!slot.used){slot=s;++records;break;}if(slot.dist<s.dist)std::swap(slot,s);idx=(idx+1)&mask;++s.dist;}}}
    ...
    Address encode(const std::string&k)const noexcept{const auto h1=compact_candidate_detail::mix64(compact_candidate_detail::fnv1a64(k));const auto h2=compact_candidate_detail::mix64(compact_candidate_detail::fnv1a64(k,1099511628211ULL)^(h1<<1));return {std::uint32_t(h1%partition_count_),h2};}
public:
