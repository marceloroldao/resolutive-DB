...
static double run_sqlite(const fs::path& p,const Data& data){
    ...
    auto t0=clock_t::now(); for(const auto& kv:data){sqlite3_bind_text(st,1,kv.first.data(),int(kv.first.size()),SQLITE_STATIC);sqlite3_bind_blob(st,2,kv.second.data(),int(kv.second.size()),SQLITE_STATIC); if(sqlite3_step(st)!=SQLITE_DONE) throw std::runtime_error("sqlite put"); sqlite3_reset(st); sqlite3_clear_bindings(st);} auto t1=clock_t::now();
...
static double run_leveldb(const fs::path& p,const Data& data){
    ...
    auto t0=clock_t::now(); for(const auto&kv:data){auto x=db->Put(wo,kv.first,kv.second);if(!x.ok())throw std::runtime_error(x.ToString());} auto t1=clock_t::now(); delete db; return data.size()/secs(t0,t1);
}
