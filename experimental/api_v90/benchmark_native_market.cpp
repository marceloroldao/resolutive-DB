...
static Data make_data(size_t n){
    Data v; v.reserve(n);
    for(size_t i=0;i<n;++i) v.emplace_back("k"+std::to_string(i), std::string(128,'A'+char(i%26)));
    return v;
}

static double secs(SteadyClock::time_point a, SteadyClock::time_point b){return std::chrono::duration<double>(b-a).count();}

static double run_bdr(const fs::path& p,const Data& data,size_t writers,size_t window){
    ...
    for(auto& t:ts)t.join(); db->sync(); auto t1=SteadyClock::now(); db->close();
    auto r=bdr::Database::open(p,o); for(const auto& kv:data){auto x=r->get(kv.first); if(!x||*x!=kv.second) throw std::runtime_error("BDR verify failed");} r->close();
    return data.size()/secs(t0,t1);
}

static double run_sqlite(const fs::path& p,const Data& data){
    ...
    auto t0=SteadyClock::now(); for(const auto& kv:data){sqlite3_bind_text(st,1,kv.first.data(),int(kv.first.size()),SQLITE_STATIC);sqlite3_bind_blob(st,2,kv.second.data(),int(kv.second.size()),SQLITE_STATIC); if(sqlite3_step(st)!=SQLITE_DONE) throw std::runtime_error("sqlite put"); sqlite3_reset(st); sqlite3_clear_bindings(st);} auto t1=SteadyClock::now();
    sqlite3_finalize(st); sqlite3_close(db); return data.size()/secs(t0,t1);
}

static double run_leveldb(const fs::path& p,const Data& data){
    ...
    auto t0=SteadyClock::now(); for(const auto&kv:data){auto x=db->Put(wo,kv.first,kv.second);if(!x.ok())throw std::runtime_error(x.ToString());} auto t1=SteadyClock::now(); delete db; return data.size()/secs(t0,t1);
}

static double run_rocksdb(const fs::path& p,const Data& data){
...
static double run_lmdb(const fs::path& p,const Data& data){
    ...
    auto t0=SteadyClock::now(); for(const auto&kv:data){MDB_txn* txn=nullptr; MDB_dbi dbi; mdb_txn_begin(env,nullptr,0,&txn); mdb_dbi_open(txn,nullptr,MDB_CREATE,&dbi); MDB_val k{kv.first.size(),const_cast<char*>(kv.first.data())},v{kv.second.size(),const_cast<char*>(kv.second.data())}; if(mdb_put(txn,dbi,&k,&v,0)!=0)throw std::runtime_error("lmdb put"); if(mdb_txn_commit(txn)!=0)throw std::runtime_error("lmdb commit");} auto t1=SteadyClock::now(); mdb_env_close(env); return data.size()/secs(t0,t1);
}

int main(){
    const Data data=make_data(20000); fs::create_directories("v90_out"); std::ofstream csv("v90_out/results.csv"); csv<<"engine,writers,window,ops_per_sec\n";
